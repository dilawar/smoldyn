...
void ChangeSize(int w, int h)
{
    ...
    if(Dimension < 3 && Fix2DAspect) {
        ...
        glLoadIdentity();
        glOrtho(ClipLeft, ClipLeft + clipwidth, ClipBot, ClipBot + clipheight, ClipFront,
            ClipBack);
        ...
        if(w >= h)
            ...
            Near = ClipSize / 2.0 / tan(FieldOfView * Aspect * PI / 180.0 / 2.0);
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
...
void KeyPush(unsigned char key, int x, int y)
{
    ...
    if(key == 'Q') {
        if(Gl2PauseState == 2) {
            if(FreeFunc)
                (*FreeFunc)(FreePointer);
            exit(0);
        }
        else
            Gl2PauseState = 2;
    }
    else if(key == ' ' && Gl2PauseState == 0)
        Gl2PauseState = 1;
    else if(key == ' ' && Gl2PauseState == 1)
        Gl2PauseState = 0;
    else if(key == 'T' && TiffNumber <= TiffNumMax) {
        glGetIntegerv(GL_VIEWPORT, viewport);
        w = viewport[2];
        h = viewport[3];
        snprintf(str, STRCHAR + 100, "%s%%0%ii.tif", TiffName,
            (int)log10((double)TiffNumMax) + 1);
        str[_STRCHAR - 1] = '\0';
    ...
    else if(key == 'z' && Dimension == 3)
        glRotatef(RotateAngle, 0, 0, 1);
    else if(key == 'Z' && Dimension == 3)
        glRotatef(-RotateAngle, 0, 0, 1);
    else if(key == 'x' && Dimension == 3)
        glRotatef(RotateAngle, 1, 0, 0);
    else if(key == 'X' && Dimension == 3)
        glRotatef(-RotateAngle, 1, 0, 0);
    else if(key == 'y' && Dimension == 3)
...
void SpecialKeyPush2(unsigned char key, int x, int y)
{
    ...
    if(Dimension < 3) {
        if(key == 'D')
            Ytrans -= (ClipRight - ClipLeft) / 100;
        else if(key == 'U')
            Ytrans += (ClipRight - ClipLeft) / 100;
        else if(key == 'R')
            Xtrans += (ClipRight - ClipLeft) / 100;
        else if(key == 'L')
            Xtrans -= (ClipRight - ClipLeft) / 100;
        ClipLeft  = ClipMidx - Xtrans - ClipSize / 2.0 / Zoom;
        ClipRight = ClipMidx - Xtrans + ClipSize / 2.0 / Zoom;
...
void SpecialKeyPush(int key, int x, int y)
{
    ...
    if(!modify) {
        if(key == GLUT_KEY_DOWN)
            SpecialKeyPush2('d', x, y);
        else if(key == GLUT_KEY_UP)
            SpecialKeyPush2('u', x, y);
        else if(key == GLUT_KEY_RIGHT)
            SpecialKeyPush2('r', x, y);
        else if(key == GLUT_KEY_LEFT)
            SpecialKeyPush2('l', x, y);
    }
    else {
        if(key == GLUT_KEY_DOWN)
            SpecialKeyPush2('D', x, y);
    ...
    char *wname, float xlo, float xhi, float ylo, float yhi, float zlo, float zhi)
{
    ...
    if(ylo == yhi && zlo == zhi)
        ...
        Dimension = 3;
    ClipSize = 1.05 * sqrt((xhi - xlo) * (xhi - xlo) + (yhi - ylo) * (yhi - ylo) +
                           (zhi - zlo) * (zhi - zlo));
    if(ClipSize == 0)
        ClipSize = 1.0;
...
float gl2GetNumber(const char *variable)
{
    ...
    if(!strcmp(variable, "ClipSize"))
        value = ClipSize;
    else if(!strcmp(variable, "ClipMidx"))
        value = ClipMidx;
    else if(!strcmp(variable, "ClipMidy"))
        value = ClipMidy;
    else if(!strcmp(variable, "ClipMidz"))
        value = ClipMidz;
    else if(!strcmp(variable, "ClipLeft"))
        value = ClipLeft;
    else if(!strcmp(variable, "ClipRight"))
        value = ClipRight;
    else if(!strcmp(variable, "ClipBot"))
        value = ClipBot;
    else if(!strcmp(variable, "ClipTop"))
        value = ClipTop;
    else if(!strcmp(variable, "ClipBack"))
        value = ClipBack;
    else if(!strcmp(variable, "ClipFront"))
        value = ClipFront;
    else if(!strcmp(variable, "FieldOfView"))
        value = FieldOfView;
    else if(!strcmp(variable, "Zoom"))
        value = Zoom;
    else if(!strcmp(variable, "Near"))
        value = Near;
    else if(!strcmp(variable, "Aspect"))
        value = Aspect;
    else if(!strcmp(variable, "PixWide"))
        value = PixWide;
    else if(!strcmp(variable, "PixHigh"))
        value = PixHigh;
    else if(!strcmp(variable, "Gl2PauseState"))
        value = (float)Gl2PauseState;
    else if(!strcmp(variable, "Dimension"))
        value = (float)Dimension;
    else if(!strcmp(variable, "Xtrans"))
        value = Xtrans;
    else if(!strcmp(variable, "Ytrans"))
        value = Ytrans;
    else if(!strcmp(variable, "Fix2DAspect"))
        value = (float)Fix2DAspect;
    else if(!strcmp(variable, "TiffNumber"))
        value = (float)TiffNumber;
    else if(!strcmp(variable, "TiffNumMax"))
        value = (float)TiffNumMax;
    else if(!strcmp(variable, "RotateAngle"))
        value = RotateAngle;
    else if(!strcmp(variable, "TiffNumberDefault"))
        value = (float)TiffNumberDefault;
    else if(!strcmp(variable, "TiffNumMaxDefault"))
        value = (float)TiffNumMaxDefault;
    else if(!strcmp(variable, "RotateAngleDefault"))
        value = RotateAngleDefault;
    else if(!strcmp(variable, "Fix2DAspectDefault"))
        value = (float)Fix2DAspectDefault;
    else
        value = 0;
    return value;
}
...
float gl2SetOptionFlt(const char *option, float value)
{
    if(!strcmp(option, "RotateAngle")) {
        if(value > 0)
            RotateAngle = (GLfloat)value;
        else
            value = (float)RotateAngle;
    }
    else
        value = -1;
    return value;
...
void gl2SetOptionStr(const char *option, const char *value)
{
    if(!strcmp(option, "TiffName")) {
        if(value)
            strncpy(TiffName, value, STRCHAR - 1);
        else
            strncpy(TiffName, TiffNameDefault, STRCHAR - 1);
...
{
    if(strchr(" QT0=-xyzXYZ", key))
        KeyPush(key, 0, 0);
    else if(strchr("durlDURL", key))
        SpecialKeyPush2(key, 0, 0);
    return;
}
...
#ifdef __gl_h_
    if(c == 'A' || c == 'a')
        glColor3ub(0x70, 0xdb, 0x93); /* aqua			*/
    else if(c == 'B' || c == 'b' || c == '6')
        glColor3ub(0x00, 0x00, 0xff); /* blue			*/
    else if(c == 'C' || c == 'c')
        glColor3ub(0x00, 0xff, 0xff); /* cyan			*/
    else if(c == 'D' || c == 'd')
        glColor3ub(0x80, 0x00, 0x00); /* dark red	*/
    else if(c == 'E' || c == 'e' || c == '8')
        glColor3ub(0x80, 0x80, 0x80); /* grey			*/
    else if(c == 'F' || c == 'f')
        glColor3ub(0xff, 0x6e, 0xc7); /* fuchsia	*/
    else if(c == 'G' || c == 'g')
        glColor3ub(0x00, 0xff, 0x00); /* green		*/
    else if(c == 'H' || c == 'h' || c == '5')
        glColor3ub(0x00, 0x80, 0x00); /* hunter		*/
    else if(c == 'I' || c == 'i')
        glColor3ub(0xdb, 0x70, 0xdb); /* indigo		*/
    else if(c == 'J' || c == 'j')
        glColor3ub(0x80, 0x80, 0x00); /* olive		*/
    else if(c == 'K' || c == 'k' || c == '0')
        glColor3ub(0x00, 0x00, 0x00); /* black		*/
    else if(c == 'L' || c == 'l')
        glColor3ub(0x32, 0xcd, 0x32); /* lime			*/
    else if(c == 'M' || c == 'm')
        glColor3ub(0xff, 0x00, 0xff); /* magenta	*/
    else if(c == 'N' || c == 'n')
        glColor3ub(0x00, 0x00, 0x80); /* navy			*/
    else if(c == 'O' || c == 'o' || c == '3')
        glColor3ub(0xff, 0xaf, 0x00); /* orange		*/
    else if(c == 'P' || c == 'p')
        glColor3ub(0x80, 0x00, 0x80); /* purple		*/
    else if(c == 'Q' || c == 'q')
        glColor3ub(0xd9, 0xd9, 0xf3); /* quartz		*/
    else if(c == 'R' || c == 'r' || c == '2')
        glColor3ub(0xff, 0x00, 0x00); /* red			*/
    else if(c == 'S' || c == 's')
        glColor3ub(0x8e, 0xdb, 0x23); /* sienna		*/
    else if(c == 'T' || c == 't')
        glColor3ub(0x00, 0x80, 0x80); /* teal			*/
    else if(c == 'U' || c == 'u')
        glColor3ub(0x20, 0x00, 0x20); /* ultraviolet */
    else if(c == 'V' || c == 'v' || c == '7')
        glColor3ub(0xee, 0x82, 0xee); /* violet		*/
    else if(c == 'W' || c == 'w' || c == '9')
        glColor3ub(0xff, 0xff, 0xff); /* white		*/
    else if(c == 'X' || c == 'x')
        glColor3ub(intrand(0xff), intrand(0xff), intrand(0xff)); /* random	*/
    else if(c == 'Y' || c == 'y' || c == '4')
        glColor3ub(0xff, 0xff, 0x00); /* yellow		*/
    else if(c == 'Z' || c == 'z')
        glColor3ub(intrand(2) * 0xff, intrand(2) * 0xff, intrand(2) * 0xff); /* random	*/
    else if(c == '1')
        glColor3ub(0x40, 0x40, 0x20); /* brown		*/
    else if(c == '-')
        glColor3ub(0xc0, 0xc0, 0xc0); /* silver		*/
    else if(c == '+')
        glColor3ub(0xff, 0xd7, 0x20); /* gold		*/
    else
        glColor3ub(0x00, 0x00, 0x00); /* black		*/
#endif
    return;
...
void gl2DrawCircle(float *cent, float radius, int slices, char style, int dim)
{
    ...
    if(dim == 2)
        ...
        for(i = 0; i < slices; i++) {
            theta = i * dtheta;
            glVertex3f(
                cent[0] + radius * cos(theta), cent[1] + radius * sin(theta), cent[2]);
        }
    if(style == 'e' || style == 'f') {
        theta = i * dtheta;
        if(dim == 2)
    ...
    int stacks, int frontin, int normals)
{
    ...
    if(baseRadius == topRadius) {
        ...
        normxy =
            (frontin ? -1.0 : 1.0) /
            sqrt((baseRadius - topRadius) * (baseRadius - topRadius) + height * height);
    ...
    for(ilen = 1; ilen <= stacks; ilen++) {
        ...
        r2 = baseRadius * (1.0 - z2 / height) +
             topRadius * z2 / height;  // r2 is top radius
        glBegin(GL_QUAD_STRIP);
        if(!normals)
    ...
    float radius, float length, int slices, int stacks, int frontin, int normals)
{
    ...
    if(length)
        gl2DrawCylinder(
            radius, radius, length, slices, stacks - 2 * endstacks, frontin, normals);
    glRotatef(180, 1, 0, 0);
    gl2DrawHemisphere(radius, slices, endstacks, frontin, normals);
    glPopMatrix();
#endif
...
/* gl2PlotPtsD */
void gl2PlotPtsD(
    double **data, int *ser, int nser, int npts, double **color, double *size, char style)
{
#ifdef __gl_h_
    int i, j;
    ...
    float *xdata, float *ydata, float **zdata, int nx, int ny, int nz, char *style)
{
    ...
    if(style[0] == 's') {
        ...
        for(iy = 0; iy < ny; iy++) {
            if(ny > 1) {
                if(iy == 0)
                    dylo = dyhi = 0.5 * (ydata[1] - ydata[0]);
                else if(iy == ny - 1)
                    dylo = dyhi = 0.5 * (ydata[ny - 1] - ydata[ny - 2]);
                else {
                ...
                dylo = dyhi = 0.5 * (ClipTop - ClipBot);
            for(ix = 0; ix < nx; ix++) {
                if(nx > 1) {
                    if(ix == 0)
                        dxlo = dxhi = 0.5 * (xdata[1] - xdata[0]);
                    else if(ix == nx - 1)
                        dxlo = dxhi = 0.5 * (xdata[nx - 1] - xdata[nx - 2]);
                    else {
                    ...
                    dxlo = dxhi = 0.5 * (ClipRight - ClipLeft);
                for(ic = 0; ic < 4; ic++)
                    col2[ic] = 0;
                for(iz = 0; iz < nz; iz++)
                    for(ic = 0; ic < 4; ic++)
                        col2[ic] += zdata[iy * nx + ix][iz] * col1[iz][ic];
                glColor4fv(col2);
                glRectf(xdata[ix] - dxlo, ydata[iy] - dylo, xdata[ix] + dxhi,
                    ydata[iy] + dyhi);
            }
        }
    }
