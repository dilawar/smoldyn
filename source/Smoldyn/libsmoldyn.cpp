...
/* smolSetLogging */
extern "C" void smolSetLogging(
    FILE *logfile, void (*logFunction)(simptr, int, const char *, ...))
{
...
/* smolSetThrowing */
extern "C" void smolSetThrowing(int corethreshold, int libthreshold)
{
    ...
    const char *errorfunction, enum ErrorCode errorcode, const char *errorstring)
{
    ...
    //??	if(LibThrowThreshold<severity) throw;	//?? This is disabled for
    // now because I can't link libsmoldyn statically if not
...
/* smolGetError */
extern "C" enum ErrorCode smolGetError(
    char *errorfunction, char *errorstring, int clearerror)
{
    enum ErrorCode erc;
...
/* smolClearError */
extern "C" void smolClearError(void)
{
    Liberrorcode        = ECok;
    Libwarncode         = ECok;
    Liberrorfunction[0] = '\0';
...
/* smolSetDebugMode */
extern "C" void smolSetDebugMode(int debugmode)
{
...
extern "C" char *smolErrorCodeToString(enum ErrorCode erc, char *string)
{
    if(erc == ECok)
        strcpy(string, "ok");
...
/* smolDisplaySim */
extern "C" enum ErrorCode smolDisplaySim(simptr sim)
{
    simsystemoutput(sim);
    checksimparams(sim);
    return ECok;
...
extern "C" simptr smolPrepareSimFromFile(
...
/* smolLoadSimFromFile */
extern "C" enum ErrorCode smolLoadSimFromFile(
...
extern "C" enum ErrorCode smolReadConfigString(
    simptr sim, const char *statement, char *parameters)
{
    return ECok;
}
...
/* smolSetSimTimes */
extern "C" enum ErrorCode smolSetSimTimes(
...
/* smolSetTimeStart */
extern "C" enum ErrorCode smolSetTimeStart(simptr sim, double timestart)
{
...
/* smolSetTimeStop */
extern "C" enum ErrorCode smolSetTimeStop(simptr sim, double timestop)
{
...
/* smolSetTimeNow */
extern "C" enum ErrorCode smolSetTimeNow(simptr sim, double timenow)
{
...
/* smolSetTimeStep */
extern "C" enum ErrorCode smolSetTimeStep(simptr sim, double timestep)
{
...
/* smolSetRandomSeed */
extern "C" enum ErrorCode smolSetRandomSeed(simptr sim, long int seed)
{
...
/* smolSetTiffParams */
extern "C" enum ErrorCode smolSetTiffParams(
...
/* smolSetLightParams */
extern "C" enum ErrorCode smolSetLightParams(simptr sim, int lightindex, double *ambient,
    double *diffuse, double *specular, double *position)
{
    ...
    LCHECK(lightindex >= 0 || (!diffuse && !specular && !position), funcname, ECsyntax,
        "can only set ambient for global light");
...
/* smolAddOutputFile */
extern "C" enum ErrorCode smolAddOutputFile(
...
/* smolAddSpecies */
extern "C" enum ErrorCode smolAddSpecies(
...
extern "C" enum ErrorCode smolSetSpeciesMobility(simptr sim, const char *species,
...
/* smolSetMolList */
extern "C" enum ErrorCode smolSetMolList(
...
extern "C" enum ErrorCode smolAddSolutionMolecules(simptr sim, const char *species,
...
extern "C" enum ErrorCode smolAddCompartmentMolecules(
...
extern "C" enum ErrorCode smolAddSurfaceMolecules(simptr sim, const char *species,
    ...
    const char *panel, double *position)
{
    ...
    LCHECK((panelshape >= 0 && panelshape < PSMAX) || panelshape == PSall, funcname,
        ECnonexist, "invalid panelshape");
    pnl = NULL;
    ...
    if(p >= 0) {
        ...
        LCHECK(
            panelshape != PSall, funcname, ECsyntax, "needs to be specific panelshape");
...
extern "C" int smolGetMoleculeCount(
...
/* smolSetBoundaryType */
extern "C" enum ErrorCode smolSetBoundaryType(
    simptr sim, int dimension, int highside, char type)
{
    ...
    LCHECK(type == 'r' || type == 'p' || type == 'a' || type == 't', funcname, ECsyntax,
        "invalid type");
...
/* smolGetSurfaceName */
extern "C" char *smolGetSurfaceName(simptr sim, int surfaceindex, char *surface)
{
    ...
    LCHECK(sim->srfss && sim->srfss->nsrf, funcname, ECnonexist, "no surfaces defined");
    LCHECK(
        surfaceindex < sim->srfss->nsrf, funcname, ECnonexist, "surface does not exist");
    ...
    enum SrfAction action)
{
    ...
    if(s == (int)ECall) {
        ...
        LCHECK(s >= 0, funcname, ECsame, NULL);
    LCHECK(face == PFfront || face == PFback || face == PFboth, funcname, ECbounds,
        "invalid face");
    i = smolGetSpeciesIndexNT(sim, species);
    if(i == (int)ECall) {
        smolClearError();
...
/* smolSetSurfaceRate */
extern "C" enum ErrorCode smolSetSurfaceRate(simptr sim, const char *surface,
    ...
    enum MolecState state2, double rate, const char *newspecies, int isinternal)
{
    ...
    LCHECK(state == MSsoln || state1 == MSsoln || state1 == MSbsoln || state1 == state,
    ...
    LCHECK(
        !(isinternal && rate > 1), funcname, ECbounds, "internal rate needs to be <= 1");

    if(s >= 0) {
        srf = sim->srfss->srflist[s];
        er  = surfsetrate(
            srf, i, NULL, state, state1, state2, i2, rate, isinternal ? 2 : 1);
...
/* smolAddPanel */
extern "C" enum ErrorCode smolAddPanel(simptr sim, const char *surface,
...
/* smolGetPanelName */
extern "C" char *smolGetPanelName(simptr sim, const char *surface,
...
/* smolSetPanelJump */
extern "C" enum ErrorCode smolSetPanelJump(simptr sim, const char *surface,
...
extern "C" enum ErrorCode smolAddSurfaceUnboundedEmitter(simptr sim, const char *surface,
...
extern "C" enum ErrorCode smolSetSurfaceSimParams(
...
/* smolSetSurfaceStyle */
extern "C" enum ErrorCode smolSetSurfaceStyle(simptr sim, const char *surface,
    enum PanelFace face, enum DrawMode mode, double thickness, double *color,
...
extern "C" char *smolGetCompartmentName(
    simptr sim, int compartmentindex, char *compartment)
{
...
extern "C" enum ErrorCode smolAddCompartmentSurface(
...
extern "C" enum ErrorCode smolAddCompartmentPoint(
...
extern "C" enum ErrorCode smolAddCompartmentLogic(
...
/* smolAddReaction */
extern "C" enum ErrorCode smolAddReaction(simptr sim, const char *reaction,
    const char *reactant1, enum MolecState rstate1, const char *reactant2,
    ...
    enum MolecState *productstates, double rate)
{
    ...
    rxn = RxnAddReaction(sim, reaction, order, rctident, rctstate, nproduct, prdident,
        productstates, NULL, NULL);
...
extern "C" int smolGetReactionIndex(simptr sim, int *orderptr, const char *reaction)
{
    ...
    if(orderptr && *orderptr >= 0 && *orderptr < MAXORDER) {
        ...
        for(order = 0; order < MAXORDER && r < 0; order++)
            if(sim->rxnss[order])
                r = stringfind(
                    sim->rxnss[order]->rname, sim->rxnss[order]->totrxn, reaction);
        LCHECK(r >= 0, funcname, ECnonexist, "reaction not found");
        if(orderptr)
            *orderptr = order - 1;
    }
...
extern "C" int smolGetReactionIndexNT(simptr sim, int *orderptr, const char *reaction)
{
    ...
    if(orderptr && *orderptr >= 0 && *orderptr < MAXORDER) {
        order = *orderptr;
        LCHECKNT(sim->rxnss[order] && sim->rxnss[order]->totrxn, funcname, ECnonexist,
            "no reactions defined of this order");
        r = stringfind(sim->rxnss[order]->rname, sim->rxnss[order]->totrxn, reaction);
...
/* smolGetReactionName */
extern "C" char *smolGetReactionName(
    simptr sim, int order, int reactionindex, char *reaction)
{
...
/* smolSetReactionRate */
extern "C" enum ErrorCode smolSetReactionRate(
...
extern "C" enum ErrorCode smolSetReactionRegion(
...
extern "C" enum ErrorCode smolSetReactionProducts(simptr sim, const char *reaction,
...
/* smolAddPort */
extern "C" enum ErrorCode smolAddPort(
    simptr sim, const char *port, const char *surface, enum PanelFace face)
{
    ...
    if(s == (int)ECmissing)
        ...
        LCHECK(s >= 0, funcname, ECsame, NULL);
    LCHECK(face == PFfront || face == PFback || face == PFnone, funcname, ECsyntax,
        "invalid face");
...
/* smolGetPortName */
extern "C" char *smolGetPortName(simptr sim, int portindex, char *port)
{
...
extern "C" enum ErrorCode smolAddPortMolecules(
...
extern "C" int smolGetPortMolecules(
...
/* smolAddLattice */
extern "C" enum ErrorCode smolAddLattice(simptr sim, const char *lattice,
...
/* smolGetLatticeName */
extern "C" char *smolGetLatticeName(simptr sim, int latticeindex, char *lattice)
{
...
extern "C" enum ErrorCode smolAddLatticeMolecules(simptr sim, const char *lattice,
...
/* smolAddLatticePort */
extern "C" enum ErrorCode smolAddLatticePort(
...
extern "C" enum ErrorCode smolAddLatticeSpecies(
...
extern "C" enum ErrorCode smolAddLatticeReaction(
