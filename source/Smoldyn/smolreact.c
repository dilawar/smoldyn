...
rxnptr RxnTestRxnExist(simptr sim, int order, const char *rname, const int *rctident,
    const enum MolecState *rctstate, int nprod, const int *prdident,
    const enum MolecState *prdstate, int exact);

// core simulation functions
int morebireact(simptr sim, rxnptr rxn, moleculeptr mptr1, moleculeptr mptr2, int ll1,
    int m1, int ll2, enum EventType et, double *vect);
        ...
        for(order = 0; order < MAXORDER && vlist == NULL; order++)
            if(sim->rxnss[order])
                for(r = 0; r < sim->rxnss[order]->totrxn; r++) {
                    ...
                    if(strbegin(rname, testname, 1) && strlen(testname) > rnamelen + 1 &&
                        testname[rnamelen] == '_' &&
                        strisnumber(testname + rnamelen + 1)) {
                        if(!rxn)
                            rxn = sim->rxnss[order]->rxn[r];  // rxn is the first one
                                                              // that's found
    ...
    if(rxntype == 3) {  // reaction rule with this name
        ...
        for(r = 0; r < sim->ruless->nrule; r++) {
            ...
            if(rule->ruletype == RTreaction &&
                !strcmp(rule->rulename, rname)) {  // found a reaction rule with this name
                molpatternindex(sim, rule->rulepattern, rname, 1, 0, &index);
                order = index[PDmatch];
                if(!rule->rulerxn) {  // create a reaction template for the rule if none
                                      // already
                    ...
                    rxn->prdstate =
                        (enum MolecState *)calloc(rxn->nprod, sizeof(enum MolecState));
...
int rxnreactantstate(rxnptr rxn, enum MolecState *mstate, int convertb2f)
{
    ...
    if(order == 0)
        ...
        if(!permit) {
            for(ms1 = (MolecState)0; ms1 < MSMAX1 && !rxn->permit[ms1 * MSMAX1 + MSsoln];
        ...
        if(!permit) {
            for(ms2 = (MolecState)0; ms2 < MSMAX1 && !rxn->permit[MSsoln * MSMAX1 + ms2];
...
int findreverserxn(simptr sim, int order, int r, int *optr, int *rptr)
{
    ...
    orderr = rrreturn = 0;
    if(order == 0 || rxn->nprod == 0 || rxn->nprod >= MAXORDER || !sim->rxnss[rxn->nprod])
        rev = 0;
    else {
        orderr  = rxn->nprod;
            ...
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                for(prd = 0; prd < rxn->nprod; prd++)
                    if(rxn->prdident[prd] == i && rxn->prdstate[prd] == ms) {
                        ...
                        if(dotVVD(rxn->prdpos[prd], rxn->prdpos[prd], sim->dim) > 0)
                            return 1;
                    }
            }
        }
...
char *rxnsernocode2string(long int pserno, char *pattern)
{
    ...
    if(bitcode == 0x0001)
        ...
        if(bitcode & 0xFF00 && bitcode & 0xFF) strcat(pattern, ".");
...
void rxnfree(rxnptr rxn)
{
    ...
    if(!rxn) return;
    if(rxn->prdpos)
        for(prd = 0; prd < rxn->nprod; prd++) free(rxn->prdpos[prd]);
    free(rxn->prdpos);
    free(rxn->prdstate);
    free(rxn->rctrep);
    free(rxn->prdrep);
            ...
            for(ll = 0; ll < maxll2o; ll++)
                if(rxnss->rxnmollist[ll]) {
                    ...
                    for(ord = 0; ord < order; ord++)
                        simLog(sim, 2, "%s%s",
                            sim->mols->listname[(ll / intpower(maxlist, ord)) % maxlist],
                            ord < order - 1 ? "+" : "");
                }
        }
        simLog(sim, 2, "\n");
        ...
        for(i = 0; i < ni2o; i++)
            if(rxnss->nrxn[i]) {
                ...
                if(Zn_issort(identlist, order) >= 1) {
                    simLog(sim, 2, "  ");
                    for(ord = 0; ord < order; ord++)
                        simLog(sim, 2, "%s%s", sim->mols->spname[identlist[ord]],
                            ord < order - 1 ? "+" : "");
                    simLog(sim, 2, " :");
    ...
    for(r = 0; r < rxnss->totrxn; r++) {
        ...
        for(rct = 0; rct < order; rct++) {
            ...
            if(rxn->rctstate[rct] != MSsoln)
                simLog(sim, 2, " (%s)", molms2string(rxn->rctstate[rct], string));
        ...
        for(prd = 0; prd < rxn->nprod; prd++) {
            ...
            if(rxn->prdstate[prd] != MSsoln)
                simLog(sim, 2, " (%s)", molms2string(rxn->prdstate[prd], string));
        ...
        if(rct == order + 1) {
            ...
            for(ms = MolecState(0); ms < nms2o; ms = MolecState(ms + 1)) {
                if(rxn->permit[ms]) {
                    ...
                    simLog(sim, 2, " ");
                    for(ord = 0; ord < order; ord++)
                        simLog(sim, 2, "%s%s", molms2string(statelist[ms], string),
                            ord < order - 1 ? "+" : "");
                }
            }
        }
        ...
        if(order == 0)
            simLog(sim, 2, "   average reactions per time step: %g\n", rxn->prob);
        else if(order == 1)
            simLog(sim, 2, "   conditional reaction probability per time step: %g\n",
                rxn->prob);  // this is the conditional probability, with the condition
                             // that prior possible reactions did not happen
        ...
        if(rxn->nprod == 2) {  // unbinding information
            if(rxn->unbindrad >= 0)
                ...
                simLog(sim, 2, "   unbinding radius: calculated from molecule overlap\n");
            else if(rxn->rparamt == RPbounce && rxn->unbindrad == -2)
                simLog(
                    sim, 2, "   unbinding radius: calculated for ballistic reflection\n");
            else
            ...
            if(rxn->rparamt != RPconfspread && rxn->rparamt != RPbounce &&
                ...
                revdsum = MolCalcDifcSum(sim, rxn->prdident[0], rxn->prdstate[0],
                ...
                if(rparamt == RPunbindrad)
                    bindradrev =
                        bindingradiusprob(revrate, 0, revdsum, rparam, 0, -1, &revprob);
                else if(rparamt == RPratio)
                    bindradrev =
                        bindingradiusprob(revrate, 0, revdsum, rparam, 1, -1, &revprob);
                else if(rparamt == RPpgem || rparamt == RPpgemmax ||
                        rparamt == RPpgemmaxw) {
                    bindradrev = bindingradiusprob(
                        revrate * (1.0 - rparam), 0, revdsum, -1, 0, -1, &revprob);
                    pgem = rparam;
                }
                else
                    bindradrev =
                        bindingradiusprob(revrate, 0, revdsum, -1, 0, -1, &revprob);
        ...
        if(order == 2 && rxnreactantstate(rxn, statelist, 1) &&
            (vflag || rxn->chi > 0 || rxn->prob < 1)) {  // compute reaction rate details
            ms1   = statelist[0];
            ms2   = statelist[1];
            i1    = rxn->rctident[0];
            i2    = rxn->rctident[1];
            ...
            rate3 = actualrate;
            if(i1 == i2)
                rate3 *= 2;  // rate3 is actual rate constant, corrected here for same
                             // reactants
            ...
            if(step > 0 && (!rev || rparamt != RPconfspread ||
                               rparamt != RPbounce)) {  // a normal bimolecular reaction
                revunbindrad =
                    (revrxn && (rparamt == RPpgem || rparamt == RPpgemmax ||
                                   rparamt == RPpgemmaxw || rparamt == RPratio ||
                                   rparamt == RPunbindrad))
                        ? revrxn->unbindrad
                        : -1;
                smolmodelrate = modelrxnrate(bindrad, revunbindrad, dsum,
                    1);  // rate for Smoluchowski model with simulation parameters
                chi           = rate3 / smolmodelrate;
                if(rxn->chi > 0)
                    simLog(sim, 2, "   requested and actual chi values: %g, %g\n",
                        rxn->chi, chi);
                else
            ...
            if(step > 0)
                simLog(sim, 1, "   step length / binding radius: %g (%s %s steps)\n",
            ...
            if(step > 0 && (!rev || rparamt != RPconfspread ||
                ...
                simLog(sim, 1,
                    "   dynamics are like two-radius Smoluchowski model with radius: "
                    "%g\n",
                    bindrad * chi);
                simLog(sim, 1,
                    "   dynamics are like C&K model with gamma' and gamma: %g, %g\n",
                    (1 - chi) / chi, bindrad * (1 - chi) / chi);
                lambdap =
                    (3.18243 * chi - 3.40864 * chi * chi + 0.984385 * chi * chi * chi) /
                    pow(1 - chi, 2.08761);
                simLog(sim, 1,
                    "   dynamics are like Doi model with lambda' and lambda: %g, %g\n",
        ...
        for(prd = 0; prd < rxn->nprod; prd++) {
            if(dotVVD(rxn->prdpos[prd], rxn->prdpos[prd], dim) > 0) {
                simLog(sim, 2,
                    "   product %s displacement:", sim->mols->spname[rxn->prdident[prd]]);
        ...
        if(rxn->nprod == 2 && sim->rxnss[2] && rxn->rparamt != RPconfspread &&
            ...
            for(j = 0; j < sim->rxnss[2]->nrxn[i]; j++) {
                ...
                if(revrxn->bindrad2 >= 0 && revrxn->rparamt != RPconfspread) {
                    dsum = MolCalcDifcSum(sim, rxn->prdident[0], rxn->prdstate[0],
                        rxn->prdident[1], rxn->prdstate[1]);
                    step = sqrt(2.0 * sim->dt * dsum);
                    pgem =
                        1.0 -
                        numrxnrateprob(step, sqrt(revrxn->bindrad2), -1, revrxn->prob) /
                            numrxnrateprob(step, sqrt(revrxn->bindrad2), rxn->unbindrad,
                                revrxn->prob);  // phi = 1-k_i/k_r
                    rev = (revrxn->nprod == order);
                    rev = rev &&
                          Zn_sameset(revrxn->prdident, rxn->rctident, identlist, order);
        ...
        if(sim->rxnss[order]) {
            ...
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                for(rct = 0; rct < order; rct++) {
                    ...
                    if(rxn->rctstate[rct] != MSsoln)
                        fprintf(fptr, "(%s)", molms2string(rxn->rctstate[rct], string));
                ...
                for(prd = 0; prd < rxn->nprod; prd++) {
                    ...
                    if(rxn->prdstate[prd] != MSsoln)
                        fprintf(fptr, "(%s)", molms2string(rxn->prdstate[prd], string));
                ...
                if(order == 1 && rxn->rctstate[0] == MSsome) {
                    for(ms = (MolecState)0; ms < MSMAX; ms = (MolecState)(ms + 1))
                        ...
                        (rxn->rctstate[0] == MSsome || rxn->rctstate[1] == MSsome)) {
                ...
                if(rxn->rate < 0) {
                    if(order == 0 && rxn->prob >= 0)
                        fprintf(
                            fptr, "reaction_production %s %g\n", rxn->rname, rxn->prob);
                    else if(order == 1 && rxn->prob >= 0)
                        fprintf(
                            fptr, "reaction_probability %s %g\n", rxn->rname, rxn->prob);
                    else if(order == 2 && rxn->bindrad2 >= 0)
                        fprintf(fptr, "binding_radius %s %g\n", rxn->rname,
                            sqrt(rxn->bindrad2));
                }
                if((order == 2 && rxn->prob != 1 && rxn->rparamt != RPconfspread) ||
                ...
                if(rxn->prdserno) {
                    ...
                    for(prd = 0; prd < rxn->nprod; prd++)
                        fprintf(
                            fptr, " %s", rxnsernocode2string(rxn->prdserno[prd], string));
                    fprintf(fptr, "\n");
                }
...
{
    int d, dim, warn, error, i1, i2, j, nspecies, r, i, ct, j1, j2, order, prd, vflag,
        prdparamflag;
    int      rev, o2, r2;
    long int pserno, bitcode;
    molssptr mols;
    double   minboxsize, vol, amax, vol2, vol3, actualrate, smolmodelrate, revunbindrad,
        chi, dsum;
    ...
    for(order = 0; order <= 2; order++) {  // condition
        ...
        if(rxnss) {
            if(rxnss->condition != SCok) {
                warn++;
                simLog(sim, 5,
                    " WARNING: order %i reaction structure %s is not up to date\n", order,
                    simsc2string(rxnss->condition, string));
            }
        }
    }
    ...
    for(order = 0; order <= 2; order++) {  // maxspecies
        ...
        if(rxnss) {
            if(!sim->mols) {
                error++;
                simLog(
                    sim, 10, " SMOLDYN BUG: Reactions are declared, but not molecules\n");
            }
            else if(sim->mols->maxspecies != rxnss->maxspecies) {
                error++;
                simLog(sim, 10,
                    " SMOLDYN BUG: number of molecule species differ between mols and "
                    "rxnss\n");
            }
        }
    }
        ...
        if(rxnss)
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                if(rxn->rxnss != rxnss) {
                    error++;
                    simLog(sim, 10,
                        " SMOLDYN BUG: Reaction %s does not list its superstructure "
                        "correctly.\n",
                        rxn->rname);
                }
            }
            ...
            for(r = 0; r < rxnss->totrxn && !prdparamflag; r++) {
                ...
                if(rxn->rparamt == RPpgemmaxw) {
                    if(!vflag) {
                        prdparamflag = 1;
                        simLog(sim, 5,
                            " WARNING: unspecified product parameter for at least one "
                            "reversible reaction.  Defaults are used.\n");
                        warn++;
                    }
                    else {
                        simLog(sim, 5,
                            " WARNING: unspecified product parameter for reversible "
                            "reaction %s.  Defaults are used.\n",
                            rxn->rname);
                        warn++;
                    }
                }
    ...
    rxnss =
        sim->rxnss[2];  // check for multiple bimolecular reactions with same reactants
    if(rxnss) {
                ...
                for(j1 = 0; j1 < rxnss->nrxn[i]; j1++) {
                    ...
                    for(j2 = 0; j2 < j1; j2++) {
                        ...
                        if(rxnallstates(rxn1) && rxnallstates(rxn2)) {
                            simLog(sim, 5,
                                " WARNING: multiply defined bimolecular reactions: "
                                "%s(all) + %s(all)\n",
                                spname[i1], spname[i2]);
                            warn++;
                        }
                                ...
                                for(ms2 = (MolecState)0; ms2 <= ms1;
                                    ...
                                    if(rxn1->permit[ms] && rxn2->permit[ms]) {
                                        simLog(sim, 5,
                                            ...
                                            "reactions: %s(%s) + %s(%s)\n",
                                            spname[i1], molms2string(ms1, string),
                                            spname[i2], molms2string(ms2, string2));
                                        warn++;
                                    }
                                }
                        }
            ...
            if(mols->difm[i1][MSsoln])
                for(i2 = 1; i2 < i1; i2++)
                    for(j = 0; j < rxnss->nrxn[i = i1 * rxnss->maxspecies + i2]; j++) {
                        ...
                        if(rxn->rate) {
                            simLog(sim, 5,
                                " WARNING: diffusion matrix for %s was ignored for "
                ...
                for(j = 0; j < rxnss->nrxn[i]; j++) {
                    ...
                    if(rxn->permit[order == 1 ? MSsoln : MSsoln * MSMAX1 + MSsoln]) {
                        for(prd = 0; prd < rxn->nprod; prd++)
                            if(rxn->prdstate[prd] != MSsoln) {
                                simLog(sim, 10,
                                    " ERROR: a product of reaction %s is surface-bound, "
                                    "but no reactant is\n",
                                    rxn->rname);
                                error++;
                            }
            ...
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                if(rxn->cmpt) {
                    if(order == 0 && rxn->cmpt->volume <= 0) {
                        simLog(sim, 10,
                            " ERROR: reaction %s cannot work in compartment %s because "
                            "the compartment has no volume\n",
            ...
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                if(rxn->srf) {
                    if(order == 0 && surfacearea(rxn->srf, sim->dim, NULL) <= 0) {
                        simLog(sim, 10,
                            " ERROR: reaction %s cannot work on surface %s because the "
                            "surface has no area\n",
        ...
        if(rxnss)
            for(r = 0; r < rxnss->totrxn; r++) {
                ...
                if(rxn->prdserno) {
                    ...
                    if(j) {
                        simLog(sim, 5,
                            " WARNING: multiple molecules might have the same serial "
                            "number due to reaction %s\n",
                            rxn->rname);
                        warn++;
                    }
                }
        ...
        for(r = 0; r < rxnss->totrxn; r++) {
            ...
            if(!rxn->srf) {
                for(prd = 0; prd < rxn->nprod; prd++)
                    if(rxn->prdstate[prd] != MSsoln) {
                        simLog(sim, 10,
                            " ERROR: order 0 reaction %s has surface-bound products but "
                            "no surface listed\n",
                            rxn->rname);
                        error++;
                    }
    ...
    if(rxnss)
        for(r = 0; r < rxnss->totrxn; r++) {
            ...
            if(rxn->prob < 0) {
                ...
                simLog(sim, 5,
                    " WARNING: order 1 reaction %s probability is at lower end of random "
                    "number generator resolution\n",
                    rxn->rname);
                warn++;
            }
            else if(rxn->prob > ((double)RAND2_MAX - 10.0) / (double)RAND2_MAX &&
                    rxn->prob < 1.0) {
                simLog(sim, 5,
                    " WARNING: order 1 reaction %s probability is at upper end of random "
                    "number generator resolution\n",
                    rxn->rname);
                warn++;
            }
            ...
            if(rxn->tau < 5 * sim->dt) {
                simLog(sim, 5,
                    " WARNING: order 1 reaction %s time constant is only %g times longer "
    ...
    if(rxnss) {
        for(r = 0; r < rxnss->totrxn; r++) {
            ...
            if(rxn->bindrad2 < 0) {
                if(rxn->rparamt == RPconfspread)
                    simLog(sim, 5,
                        " WARNING: confspread radius not set for order 2 reaction %s\n",
                        rxn->rname);
                else
                    simLog(sim, 5,
                        " WARNING: reaction rate not set for reaction order 2, name %s\n",
                        rxn->rname);
                rxn->bindrad2 = 0;
                warn++;
            }
                ...
                if(rxn->rparamt == RPconfspread)
                    simLog(sim, 5,
                        " WARNING: confspread radius for order 2 reaction %s is larger "
                        "than box dimensions\n",
                        rxn->rname);
                else if(rxn->rparamt == RPbounce)
                    simLog(sim, 5,
                        " WARNING: bounce radius for order 2 reaction %s is larger than "
                        "box dimensions\n",
                        rxn->rname);
                else
                    simLog(sim, 5,
                        " WARNING: binding radius for order 2 reaction %s is larger than "
                        "box dimensions\n",
                        rxn->rname);
                warn++;
            }
            ...
            if(rxn->tau < 5 * sim->dt) {
                simLog(sim, 5,
                    " WARNING: order 2 reaction %s time constant is only %g times longer "
                    "than the simulation time step\n",
                    rxn->rname, rxn->tau / sim->dt);
                warn++;
            }
            if(rxn->chi > 0) {
                ...
                dsum = MolCalcDifcSum(
                    sim, rxn->rctident[0], statelist[0], rxn->rctident[1], statelist[1]);
                ...
                if(chi < 0.98 * rxn->chi) {
                    warn++;
                    simLog(sim, 5,
                        " WARNING: order 2 reaction %s cannot achieve requested chi of "
                        "%g but is only %g\n",
...
int rxnsetrate(simptr sim, int order, int r, char *erstr)
{
    ...
    if(rxn->disable) {  // disabled reaction
        ...
        if(rxn->rate < 0) {
            snprintf(erstr, STRCHAR * sizeof(erstr), "reaction %s rate is undefined",
                rxn->rname);
            return 1;
        }
        if(rxn->rate >= 0)
        ...
        for(ms = (MolecState)0; ms < MSMAX && !rxn->permit[ms]; ms = (MolecState)(ms + 1))
            ;  // determine first state that this is permitted for (usually only state)
        if(rxn->rate < 0) {
            snprintf(erstr, STRCHAR * sizeof(erstr), "reaction %s rate is undefined",
                rxn->rname);
            return 1;
                ...
                if(rxn2->permit[ms] && rxn2->rate > 0) {
                    for(ms1 = (MolecState)0; ms1 < MSMAX; ms1 = (MolecState)(ms1 + 1))
                        if(rxn->permit[ms1] != rxn2->permit[ms1]) {
                            snprintf(erstr, STRCHAR * sizeof(erstr),
                                "reactions %s and %s have the same reactant but "
                                "different state permissions, which is not allowed",
            ...
            rxn->prob = rxn->rate * rxn->multiplicity / sum *
                        (1.0 - exp(-sum * sim->dt));  // desired reaction probability

            product = 1.0;
            for(j = 0; j < rxnss->nrxn[i];
                j++) {  // increase reaction probability to account for prior reactions in
                        // the list
                rxn2 = rxnss->rxn[rxnss->table[i][j]];
                if(rxn2 == rxn) break;
                if(rxn2->permit[ms]) {
            ...
            rxn->prob /= product;  // probability, accounting for prior reactions, see
                                   // documentation
            if(!(rxn->prob >= 0 && rxn->prob <= 1)) {
                snprintf(erstr, STRCHAR * sizeof(erstr),
                    "reaction %s probability is %g, which is out of range", rxn->rname,
                    rxn->prob);
                return 5;
            }
        }
        ...
        if(rxn->rate < 0) {
            if(rxn->prob < 0) rxn->prob = 1;
            snprintf(erstr, STRCHAR * sizeof(erstr), "reaction rate %s is undefined",
                rxn->rname);
            return 1;
        }
        i1     = rxn->rctident[0];
        ...
        dsum = MolCalcDifcSum(sim, i1, ms1, i2, ms2);
        rev  = findreverserxn(
            sim, 2, r, &o2, &r2);  // rev is 0 for irreversible, 1 for direct reverse
                                    // reaction, 2 for continuation reaction
        ...
        if(!permit)
            ...
            snprintf(erstr, STRCHAR * sizeof(erstr), "Both diffusion coefficients are 0");
            return 4;
        }
        else if(rparamt == RPunbindrad)
            rxn->bindrad2 =
                bindingradiusprob(rate3, sim->dt, dsum, rparam, 0, rxn->chi, &rxn->prob);
        else if(rparamt == RPratio)
            rxn->bindrad2 =
                bindingradiusprob(rate3, sim->dt, dsum, rparam, 1, rxn->chi, &rxn->prob);
        else if(rparamt == RPpgem)
            rxn->bindrad2 = bindingradiusprob(
                rate3 * (1.0 - rparam), sim->dt, dsum, -1, 0, rxn->chi, &rxn->prob);
        else if(rparamt == RPpgemmax || rparamt == RPpgemmaxw) {
            rxn->bindrad2 =
                bindingradiusprob(rate3, sim->dt, dsum, 0, 0, rxn->chi, &rxn->prob);
            ...
            rxn->bindrad2 = bindingradiusprob(rate3, sim->dt, dsum, -1, 0, rxn->chi,
                &rxn->prob);  // irreversible or assumed irreversible
...
int rxnsetproduct(simptr sim, int order, int r, char *erstr)
{
    ...
    if(nprod == 0) {  // nprod==0
        ...
        if(rpar >= 0) {
            ...
            for(prd = 2; prd < nprod; prd++) rxn->prdpos[prd][0] = 0;
        }
        else if(rpar < 0) {
            rxn->unbindrad    = rpar;
        ...
        if(rparamt == RPirrev ||
            ...
            for(prd = 0; prd < nprod; prd++)
                for(d = 0; d < dim; d++) rxn->prdpos[prd][d] = 0;
        }

        else if(rev == 0) {  // nprod>=2, irreversible, not offset, fixed, irrev,
                             // confspread, or bounce
            if(rparamt == RPpgem || rparamt == RPpgemmax || rparamt == RPpgemmaxw ||
                rparamt == RPratio || rparamt == RPpgem2 || rparamt == RPpgemmax2 ||
                rparamt == RPratio2) {
                snprintf(erstr, STRCHAR * sizeof(erstr),
            ...
            if(rparamt == RPnone) {
                ...
                snprintf(erstr, STRCHAR * sizeof(erstr),
                    "Cannot set unbinding distance because sum of product diffusion "
                    "constants is 0");
                er = 4;
            }
                ...
                if(rpar == -2) {
                    ...
                    snprintf(erstr, STRCHAR * sizeof(erstr),
                        "Maximum possible geminate binding probability is %g", -rpar);
                    er = 8;
                }
                else {
                    rxn->unbindrad    = rpar;
                ...
                rpar = unbindingradius(rpar, sim->dt, dsum, bindradr);
                if(rpar == -2) {
                    snprintf(erstr, STRCHAR * sizeof(erstr), "Illegal input values");
                    er = 9;
                }
                else if(rpar <= 0) {
                    rxn->unbindrad = 0;
...
double rxncalcrate(simptr sim, int order, int r, double *pgemptr)
{
    ...
    if(order == 0) {  // order 0
        ...
        if(rxn->prob > 0 && ms1 != MSMAX) {
            ...
            for(j = 0; j < rxnss->nrxn[i1];
                ...
                if(rxn2->permit[ms1] && rxn2->prob > 0) {
                    prob =
                        rxn2->prob * product;  // prob is probability that rxn2 is chosen
        ...
        if(rxn->bindrad2 < 0 || rxn->prob < 0)
            ...
            if(rparamt == RPpgem || rparamt == RPpgemmax || rparamt == RPpgemmaxw ||
                rparamt == RPratio ||
                rparamt == RPunbindrad) {  // list was changed in version 2.56 to agree
                                           // with Smoldyn User's manual
...
int RxnSetValue(simptr sim, const char *option, rxnptr rxn, double value)
{
    ...
    if(!strcmp(option, "rate")) {
        if(value < 0) er = 4;
        rxn->rate = value;
        if(rxn->multiplicity < 0) rxn->multiplicity = 1;
    }
    ...
    if(sim) rxnsetcondition(sim, -1, SClists, 0);
...
int RxnSetValuePattern(simptr sim, const char *option, const char *pattern,
    const char *rname, const enum MolecState *rctstate, const enum MolecState *prdstate,
    double value, int oldnresults, const rxnptr templ)
{
    ...
    for(j = oldnresults; j < nresults; j++) {
        ...
        rxn = RxnTestRxnExist(
            sim, matchwords, rname, rctident, rctstate, subwords, prdident, prdstate, 0);
        if(!rxn) return 5;
        if(!strcmp(option, "rate")) {
            if(rxn->rate < 0)
        ...
        if(templ) {
            if(templ->rate >= 0) RxnSetValue(sim, "rate", rxn, templ->rate);
            ...
            if(templ->prob >= 0) RxnSetValue(sim, "prob", rxn, templ->prob);
            if(templ->prdserno) RxnSetPrdSerno(rxn, templ->prdserno);
            if(templ->prdintersurf) RxnSetIntersurfaceRules(rxn, templ->prdintersurf);
            if(templ->rctrep)
            ...
            if(templ->rparamt != RPnone) RxnCopyRevparam(sim, rxn, templ);
        }
    }
...
/* RxnSetRevparam */
int RxnSetRevparam(simptr sim, rxnptr rxn, enum RevParam rparamt, double rparam, int prd,
    double *pos, int dim)
{
    int d, er;
...
void RxnCopyRevparam(simptr sim, rxnptr rxn, const rxnptr templ)
{
    ...
    for(prd = 0; prd < rxn->nprod; prd++)
        for(d = 0; d < sim->dim; d++) rxn->prdpos[prd][d] = templ->prdpos[prd][d];
...
int RxnSetPrdSerno(rxnptr rxn, long int *prdserno)
{
    ...
    if(!rxn->prdserno) {
        ...
        for(prd = 0; prd < rxn->nprod; prd++) rxn->prdserno[prd] = 0;
    }

    for(prd = 0; prd < rxn->nprod; prd++) rxn->prdserno[prd] = prdserno[prd];
...
int RxnSetIntersurfaceRules(rxnptr rxn, int *rules)
{
    ...
    if(!rxn->prdintersurf) {
        ...
        for(prd = 0; prd < rxn->nprod; prd++) rxn->prdintersurf[prd] = 0;
    }
    ...
    if(rules[0] == -1) {
        ...
        for(prd = 0; prd < rxn->nprod; prd++) rxn->prdintersurf[prd] = rules[prd];
...
int RxnSetRepresentationRules(rxnptr rxn, int order,
    const enum SpeciesRepresentation *rctrep, const enum SpeciesRepresentation *prdrep)
{  // used for lattice/particle hybrid
    ...
    if(!rxn->rctrep) {
        ...
        for(rct = 1; rct < order; rct++) rxn->rctrep[rct] = SRparticle;
    }
    if(!rxn->prdrep) {
        ...
        for(prd = 1; prd < nprod; prd++) rxn->prdrep[prd] = SRparticle;
    }
    ...
    for(rct = 0; rct < order; rct++) rxn->rctrep[rct] = rctrep[rct];
    for(prd = 0; prd < nprod; prd++) rxn->prdrep[prd] = prdrep[prd];
...
rxnptr RxnAddReaction(simptr sim, const char *rname, int order, int *rctident,
    enum MolecState *rctstate, int nprod, int *prdident, enum MolecState *prdstate,
    compartptr cmpt, surfaceptr srf)
{
    ...
    if(r >= 0) {
        CHECKBUG(rxnss->rxn[r]->nprod == 0,
            "RxnAddReaction cannot be called for a reaction that already has products");
        rxn = rxnss->rxn[r];
    }
    else {
    ...
    if(nprod) {
        ...
        CHECKMEM(
            rxn->prdstate = (enum MolecState *)calloc(nprod, sizeof(enum MolecState)));
    ...
    if(vflag) {
        ...
        for(rct = 0; rct < order; rct++)
            simLog(sim, 1, " %s(%s) %s", sim->mols->spname[rctident[rct]],
                molms2string(rctstate[rct], string), rct < order - 1 ? "+" : "->");
        for(prd = 0; prd < nprod; prd++)
            simLog(sim, 1, " %s(%s)%s", sim->mols->spname[prdident[prd]],
                molms2string(prdstate[prd], string), prd < nprod - 1 ? " +" : "");
        simLog(sim, 1, "\n");
    }
    ...
    const enum MolecState *prdstate, int exact)
{
    ...
    if(order == 0) {
        for(j = 0; j < rxnss->totrxn; j++) {
            rxn = rxnss->rxn[j];  // rxn is same if same name or name portion and same
                                  // reactants and same products
                ...
                if(chptr[0] == '\0' || (chptr[0] == '_' && strisnumber(chptr + 1)))
                    if(nprod == rxn->nprod)
                        if(Zn_sameset(prdident, rxn->prdident, work, nprod) &&
                            Zn_sameset(
                                (int *)prdstate, (int *)(rxn->prdstate), work, nprod))
                            return rxn;
            }
        }
    }
        ...
        for(j = 0; j < rxnss->nrxn[i]; j++) {
            ...
            if((chptr = strstr(rxn->rname, rname))) {
                ...
                if(chptr[0] == '\0' || (chptr[0] == '_' && strisnumber(chptr + 1)))
                    if(nprod == rxn->nprod)
                        if(Zn_sameset(
                               (int *)rctstate, (int *)(rxn->rctstate), work, order))
                            if(Zn_sameset(prdident, rxn->prdident, work, nprod) &&
                                Zn_sameset(
                                    (int *)prdstate, (int *)(rxn->prdstate), work, nprod))
                                return rxn;
    ...
    compartptr cmpt, surfaceptr srf, int isrule, rxnptr *rxnpt)
{
    ...
    if(oldnresults < 0) {
        molpatternindex(
            sim, pattern, rname, isrule, 0, &index);  // get oldnresults from index
        if(index)
    ...
    for(j = oldnresults; j < nresults;
        ...
        if(nresults > 1)
            snprintf(
                rnamelocal + strlen(rnamelocal), STRCHAR - strlen(rnamelocal), "_%i", j);
        for(iword = 0; iword < matchwords; iword++)
            rctident[iword] = index[PDMAX + j * totalwords + iword];
        for(iword = 0; iword < subwords; iword++)
            prdident[iword] = index[PDMAX + j * totalwords + matchwords + iword];
        rxn = RxnTestRxnExist(
            sim, matchwords, rname, rctident, rctstate, subwords, prdident, prdstate, 0);
        if(!rxn) {
            rxn = RxnAddReaction(sim, rnamelocal, matchwords, rctident, rctstate,
                subwords, prdident, prdstate, cmpt, srf);
            if(!rxn) return -30;
        }
    }
    ...
    compartptr cmpt, surfaceptr srf)
{
    ...
    for(i = 0; i < order; i++) {
        ...
        CHECKBUG(
            rctstate[i] >= 0 && rctstate[i] < MSMAX1, "reactant state out of bounds");
    }
    ...
    for(i = 0; i < nprod; i++) {
        CHECKBUG(prdident[i] > 0 && prdident[i] < sim->mols->nspecies,
            "reactant identity out of bounds");
        CHECKBUG(
            prdstate[i] >= 0 && prdstate[i] < MSMAX1, "reactant state out of bounds");
    }
    if(cmpt) {
    ...
    rxn = RxnAddReaction(
        sim, rname, order, rctident, rctstate, nprod, prdident, prdstate, cmpt, srf);
    return rxn;
failure:
    simLog(sim, 10, "%s", ErrorString);
    return NULL;
        ...
        if(sim->rxnss[order] && sim->rxnss[order]->condition <= SCparams) {
            ...
            if(er >= 0) {
                simLog(sim, 8,
                    ...
                    sim->rxnss[order]->rname[er], errorstr);
                return 3;
            }
        }
            ...
            if(order == 1) {
                ...
                for(ms1 = (MolecState)0; ms1 < MSMAX1; ms1 = (MolecState)(ms1 + 1))
                    for(ms2 = (MolecState)0; ms2 < MSMAX1; ms2 = (MolecState)(ms2 + 1)) {
                        ...
                        {
                            ll1 =
                                sim->mols->listlookup[i1][ms1 == MSbsoln ? MSsoln : ms1];
                            ll2 =
                                sim->mols->listlookup[i2][ms2 == MSbsoln ? MSsoln : ms2];
...
int rxnparsereaction(simptr sim, const char *word, char *line2, char *errstr)
{
    ...
    else if(!strcmp(word, "reaction_cmpt")) {  // reaction_cmpt: deprecated but still
                                               // supported
    ...
    else if(!strcmp(word, "reaction_surface")) {  // reaction_surface: deprecated but
                                                  // still supported
    ...
    for(order = 0; order < MAXORDER; order++)  // check for whether reaction name is new
        if(sim->rxnss[order]) {
            CHECKS(stringfind(
                       sim->rxnss[order]->rname, sim->rxnss[order]->totrxn, rname) < 0,
    ...
    while(more) {  // product list
        ...
        if(strcmp(nm, "0")) {
            CHECKS(size_t(nprod + 1) < MAXPRODUCT,
            ...
            CHECKS(er != -3, "molecule state could not be read");
            CHECKS(er != -4,
                "reactant and product list together exceed maximum string length");
            CHECKS(ms < MSMAX1, "invalid product state");
            CHECKS(!(order == 0 && srf == NULL && ms != MSsoln),
                "enter a surface for order 0 reactions with surface-bound products");
            prdstate[nprod] = ms;
            nprod++;
        }
        else if(nprod == 0) {
        ...
        if(itct == 1) {
            ...
            more = 0;
            CHECKS(
                line2 = strnword(line2, 2), "SMOLDYN BUG: error in reading product list");
        }
        else
    ...
    if(!isrule) {
        er = RxnAddReactionPattern(
            sim, rname, pattern, -1, rctstate, prdstate, cmpt, srf, isrule, &rxn);
    ...
    if(bidirect) {
        ...
        CHECKS(er != -5,
            "the reverse reaction would have more than two reactants, which is not "
            "allowed");
        CHECKS(!er, "BUG: error in reverse reaction code");
        if(!isrule) {
            er = RxnAddReactionPattern(sim, rnamerev, patternrev, -1, prdstate, rctstate,
                cmpt, srf, isrule, &rxnrev);
            CHECKS(er != -1, "out of memory adding reverse reaction");
            if(!isrule) {
                CHECKS(er != -2, "one of the reverse reaction reactant names is unknown");
            }
            if(!isrule) {
                CHECKS(er != -3, "one of the reverse reaction product names is unknown");
            }
            ...
            CHECKS(er != -12,
                "BUG: missing reactant in wildcard expression for reverse reaction");
            CHECKS(er != -13,
                "missing AND operand in wildcard expression for reverse reaction");
            CHECKS(er != -15,
                "mismatched braces in wildcard expression for reverse reaction");
            CHECKS(er != -20,
                "reactant and product wildcard logic don't correspond for reverse "
                "reaction");
    ...
    if(line2) {
#ifdef OPTION_VCELL
        CHECKS(
            rxn, "wildcards and species groups are not supported in reactions in VCell");
        ...
        if(found != string::npos) {
            ...
            // if rate is a exp, a "fake" set value has to be set to allow RxnSetRate to
            // do proper job, especially when there is reversible reactions, the
            // rxn->rparamt, and rxn->bindrad2 have to be set.
...
#endif
        {
            ...
            if(!isrule) {
                if(rxn)
                    ...
                    er = RxnSetValuePattern(
                        sim, "rate", pattern, rname, rctstate, prdstate, flt1, 0, NULL);
            ...
            if(bidirect && line2) {
                ...
                if(!isrule) {
                    if(rxnrev)
                        ...
                        er = RxnSetValuePattern(sim, "rate", patternrev, rnamerev,
                            prdstate, rctstate, flt2, 0, NULL);
    ...
    if(isrule) {
        ...
        detailsi[1] = s;
        er          = RuleAddRule(
            sim, RTreaction, rname, pattern, rctstate, prdstate, flt1, detailsi, NULL);
        CHECKS(!er, "out of memory or other error adding rule");
        if(bidirect) {
            er = RuleAddRule(sim, RTreaction, rnamerev, patternrev, prdstate, rctstate,
                flt2, detailsi, NULL);
...
/* doreact */
int doreact(simptr sim, rxnptr rxn, moleculeptr mptr1, moleculeptr mptr2, int ll1, int m1,
    int ll2, int m2, double *pos, panelptr rxnpnl)
{
    ...
    for(prd = 0; prd < nprod; prd++) {
        ...
        if(rxn->rparamt == RPconfspread) {  // confspread reaction
            if(prd < 2) {
                ...
                if(mptr->box) mptr->box = mptrfrom->box;
                ...
                mptrfrom = mptr1;
                for(d = 0; d < dim; d++) mptr->posx[d] = mptr->pos[d] = rxnpos[d];
                if(ms == MSsoln)
            ...
            if(prd == 0) {
                ...
                if(rxn->rparam == -2) {  // reflection method
                    pvalue = Geo_SphereReflectSphere(mptr1->posx, mptr1->pos, mptr2->posx,
                        mptr2->pos, dim, rxn->bindrad2, v1, v2);
                ...
                if(pvalue == -1) {
                    ...
                    for(d = 0; d < dim; d++) {
                        v1[d] =
                            mptr2->pos[d] - mptr1->pos[d];  // v1 is from mptr1 to mptr2
                        dist += v1[d] * v1[d];
                    ...
                    if(dist == 0) {
                        ...
                        dist = sqrt(dist);
                    if(rxn->rparam >= 0)
                        x = 1.0 / dist - (1.0 / rxn->rparam);  // x is scaling factor
                    else
                        x = 2.0 / dist - 2.0 / (rxn->prdpos[0][0] + rxn->prdpos[1][0]);
                    for(d = 0; d < dim; d++)
            ...
            if(!rxnpnl)
                ...
                fixpt2panelnocross(
                    sim, mptr->posx, mptr->pnlx, dim, PFfront, sim->srfss->epsilon);
                mptr->pnl = NULL;
            }
            else if(ms == MSbsoln) {  // surf -> back soln
                fixpt2panelnocross(
                    sim, mptr->posx, mptr->pnlx, dim, PFback, sim->srfss->epsilon);
                mptr->mstate = MSsoln;
                mptr->pnl    = NULL;
            }
                ...
                fixpt2panelnocross(
                    sim, mptr->posx, mptr->pnlx, dim, PFnone, sim->srfss->epsilon);
            }
            ...
            if(d != dim) {
                ...
                for(d = 0; d < dim; d++) mptr->pos[d] = mptr->posx[d] + v1[d];
            }
            else {
                for(d = 0; d < dim; d++) mptr->pos[d] = mptr->posx[d];
            }

            if(mptr->mstate != MSsoln) movemol2closepanel(sim, mptr);

            if(rxn->prdserno) {  // set product serial numbers
                pserno         = rxn->prdserno[prd];
                mptr->serno    = molfindserno(sim, mptr->serno, pserno,
                    mptr1 ? mptr1->serno : 0, mptr2 ? mptr2->serno : 0, sernolist);
                sernolist[prd] = mptr->serno;
            }
        }

        if(rxn->logserno) {  // log reaction if needed
            if(dorxnlog == 0 && (ListMemberLI(rxn->logserno, mptr->serno & 0xFFFFFFFF) ||
        ...
        if(rxn->rateValueProvider != NULL) {
            if(rxn->cmpt) {
                ...
                for(i = 0; i < totalNumMesh; i++) {
                    ...
                    if(posincompart(sim, centerPos, rxn->cmpt, 0)) {
                        ...
                        while(count < nmol) {
                            ...
                            // since we are using vcell mesh, we double check the pos in
                            // compartment
                            if(posincompart(sim, pos, rxn->cmpt, 0)) {
                                // count++;
                                if(doreact(
                                       sim, rxn, NULL, NULL, -1, -1, -1, -1, pos, pnl))
                                    return 1;
                            }
                        }
                ...
                for(i = 0; i < numOfSuf; i++) {
                    if(surfaces[i] == rxn->srf) {  // find the surface where the reaction
                                                   // happens
                        ...
                        for(long j = 0; j < numOfTriPanels; j++) {
                            ...
                            while(count < nmol) {
                                ...
                                panelrandpos(panels[j], pos,
                                    sim->dim);  // randomly generate a position for
                                                // surface molecule
                                ////since we are using vcell mesh, we double check the pos
                                /// in compartment
                                // if(posincompart(sim,pos,rxn->cmpt,0))
                                //{
                                // count++;
                                if(doreact(sim, rxn, NULL, NULL, -1, -1, -1, -1, pos,
                                       panels[j]))
                                    return 1;
                                //}
                            }
        ...
        if(rxnss->rxnmollist[ll]) {
            ...
            for(m = 0; m < nmol; m++) {
                ...
                for(j = 0; j < nrxn[i]; j++) {
                    ...
                    if(rxn->rateValueProvider != NULL) {
                        if(rxn->srf)  // surface reaction
                        {
                            if(mptr->pnl) {
                                ...
                                simLog(sim, 10,
                                    "Unimolecular membrance reaction should have a "
                                    "membrane reactant.");
                            }
                        }
                        else {
                    ...
                    else if(rxn->cmpt && !posincompart(sim, mptr->pos, rxn->cmpt, 0))
                        ;  // failed compartment test
                    else if(rxn->srf && (!mptr->pnl || mptr->pnl->srf != rxn->srf))
                        ;  // failed surface test
                    else if(mptr->ident == 0)
                        ;   // failed existance test
                    else {  // react
                        if(doreact(sim, rxn, mptr, NULL, ll, m, -1, -1, NULL, NULL))
                            return 1;
...
int RxnHybridReact(simptr sim)
{
    ...
    for(r = 0; r < rxnss->totrxn; r++) {
        ...
        if(rxn->rctrep && rxn->rctrep[0] != rxn->rctrep[1]) {
            ...
            partms = rxn->rctstate[partrct];  // state of particle reactant (must be
            ...
            for(m = 0; m < mols->nl[ll]; m++) {
                ...
                if(i == parti && ms == partms) {
                    latconc =
                        NSV_CALL(nsv_concentration_point(nsv, lati, mptr->pos, dim));
...
//?? check to make sure that particle reactant states are specific states, not none, all,
// or some
    ...
    int m1, int ll2, enum EventType et, double *vect)
{
    ...
    if(rxn->srf && !((mptr1->pnl && mptr1->pnl->srf == rxn->srf) ||
        ...
        return 0;
    if(rxn->rctrep && (rxn->rctrep[0] == SRlattice || rxn->rctrep[1] == SRlattice))
        return 0;
    ...
    if(msA == MSsoln && msB != MSsoln)
        msA = (panelside(mptrA->pos, mptrB->pnl, sim->dim, NULL, 0, 0) == PFfront)
                  ? MSsoln
                  : MSbsoln;
    else if(msB == MSsoln && msA != MSsoln)
        msB = (panelside(mptrB->pos, mptrA->pnl, sim->dim, NULL, 0, 0) == PFfront)
                  ? MSsoln
                  : MSbsoln;
    ms = (MolecState)(msA * MSMAX1 + msB);

    if(rxn->permit[ms]) {
        if(et == ETrxn2wrap && rxn->rparamt != RPconfspread) {  // if wrapping, then move
                                                                // faster diffusing
                                                                // molecule
    ...
    if(rxn->rateValueProvider != NULL) {
        ...
        if(rxn->srf)  // surface reaction
        {
            if(mptr1->pnl) {
                ...
                simLog(sim, 10,
                    "Bimolecular membrance reaction should have at least one membrane "
                    "reactant.");
            }
        }
        else {
...
{
    int dim, maxspecies, ll1, ll2, i, j, d, *nl, nmol2, b2, m1, m2, bmax, wpcode, nlist,
        maxlist;
    int *         nrxn, **table;
                        ...
                        for(m2 = 0; m2 < nmol2 && mlist2[m2] != mptr1; m2++) {
                            ...
                            for(j = 0; j < nrxn[i]; j++) {
                                ...
                                if(dist2 <= rxn->bindrad2 &&
                                    ...
                                    (mptr1->mstate != MSsoln || mptr2->mstate != MSsoln ||
                                        !rxnXsurface(sim, mptr1, mptr2)) &&
                                    mptr1->ident != 0 && mptr2->ident != 0) {
                                    if(morebireact(sim, rxn, mptr1, mptr2, ll1, m1, ll2,
                                           ETrxn2intra, NULL))
                                        loopbreak_ = true;
                                    if(mptr1->ident == 0 && (! loopbreak_)) {
                                        j  = nrxn[i];
                                        m2 = nmol2;
                                    }
                                }
                ...
                if(rxnss->rxnmollist[ll1 * maxlist + ll2]) {
                    for(m1 = 0; m1 < nl[ll1]; m1++) {
                        ...
                        for(b2 = 0; b2 < bmax; b2++) {
                            ...
                            if(bptr->wpneigh && bptr->wpneigh[b2]) {  // neighbor box with
                                                                      // wrapping
...
#endif
                                        dist2 = wallcalcdist2(
                                            sim, mptr1->pos, mptr2->pos, wpcode, vect);
                                        if(dist2 <= rxn->bindrad2 &&
                                            (rxn->prob == 1 || randCOD() < rxn->prob) &&
                                            mptr1->ident != 0 && mptr2->ident != 0) {
                                            if(morebireact(sim, rxn, mptr1, mptr2, ll1, m1, ll2, ETrxn2wrap, vect))
                                                loopbreak_ = false;
                                            if(mptr1->ident == 0 && (! loopbreak_)) {
                                                j  = nrxn[i];
                                                m2 = nmol2;
                            ...
                            else if(! loopbreak_) {  // neighbor box, no wrapping
                                for(m2 = 0; m2 < nmol2; m2++) {
                                    mptr2 = mlist2[m2];
                                    i     = mptr1->ident * maxspecies + mptr2->ident;
                                    for(j = 0; j < nrxn[i]; j++) {
                                        ...
                                        if(dist2 <= rxn->bindrad2 &&
                                            ...
                                            (mptr1->mstate != MSsoln ||
                                                ...
                                                !rxnXsurface(sim, mptr1, mptr2)) &&
                                            mptr1->ident != 0 && mptr2->ident != 0) {
                                            if(morebireact(sim, rxn, mptr1, mptr2, ll1, m1, ll2, ETrxn2inter, NULL))
                                                loopbreak_ = true;
                                            if(mptr1->ident == 0 && (! loopbreak_)) {
                                                j  = nrxn[i];
                                                m2 = nmol2;
