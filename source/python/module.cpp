 ...
 */
/* ----------------------------------------------------------------------------*/
int init_and_run(const string &filepath, const string &flags, bool wflag,
                 bool quit_at_end = false)
{
    ...
    if(!er) {
        ...
        if(pSim->graphss && pSim->graphss->graphics != 0) gl2glutInit(0, nullptr);
        er = simUpdateAndDisplay(pSim);
    }
    if(!er) er = smolOpenOutputFiles(pSim, wflag);
    if(er) {
...
PYBIND11_MODULE(_smoldyn, m)
{
    ...
    py::class_<graphicssuperstruct>(m, "Graphics")
        ...
        .def_readonly("method",
                      &graphicssuperstruct::graphics)  // graphics: 0=none, 1=opengl,
                                                       // 2=good opengl
        .def_readonly("runmode",
                      &graphicssuperstruct::runmode)  // 0=Smoldyn, 1=Libsmoldyn
        .def_readonly("currentIter",
                      &graphicssuperstruct::currentit)  // current number of simulation
                                                        // time steps
        .def_readonly("graphicIter",
                      &graphicssuperstruct::graphicit)  // number of time steps per
                                                        // graphics update
        ...
        .def_readonly("gridPoints",
                      &graphicssuperstruct::gridpts)  // thickness of virtual box grid for
                                                      // graphics
    ...
    py::class_<simstruct>(m, "simstruct")
        ...
        // These are readonly.
        .def_readonly("clockstt", &simstruct::clockstt,
                      "clock starting time of simulation")
        .def_readonly("elapsedtime", &simstruct::elapsedtime,
                      "elapsed time of simulation")
        .def_readonly("randseed", &simstruct::randseed, "random number generator seed")
        .def_readonly("eventcount", &simstruct::eventcount,
     ...
     *
     * It contains a self-contained model. Each object of this class has its own private
     * simptr. This is an standalone unit of simulation.
     *
     ...
     * friendly error messages and an object-oriented design. It is possible to build
     * a complete smoldyn model using this module only.
     */
    py::class_<Simulation>(m, "Simulation")
        ...
        .def("setTiffParams",
             [](Simulation &sim, int timesteps, const char *tiffname, int lowcount,
                int highcount) {
                 return smolSetTiffParams(sim.getSimPtr(), timesteps, tiffname, lowcount,
                                          highcount);
             })
        ...
        .def("addCommand",
             [](Simulation &sim, char type, double on, double off, double step,
                double multiplier, const char *commandstring) {
                 return smolAddCommand(sim.getSimPtr(), type, on, off, step, multiplier,
                                       commandstring);
             })
        ...
        .def("getSpeciesName",
             [](Simulation &sim, int speciesindex, char *species) {
                 return smolGetSpeciesName(sim.getSimPtr(), speciesindex, species);
             })
        ...
        .def(
            "setSpeciesMobility",
            [](Simulation &sim, const char *species, MolecState state, double difc,
               vector<double> &drift, vector<double> &difmatrix) {
                return smolSetSpeciesMobility(sim.getSimPtr(), species, state, difc,
                                              &drift[0], &difmatrix[0]);
            },
        ...
        .def("getMolListName",
             [](Simulation &sim, int mollistindex, char *species) {
                 return smolGetMolListName(sim.getSimPtr(), mollistindex, species);
             })
        ...
        .def(
            ...
            [](Simulation &sim, const char *species, size_t number,
               vector<double> &lowposition, vector<double> &highposition) {
                return smolAddSolutionMolecules(sim.getSimPtr(), species, number,
        ...
        //     *compartment);
        .def("addCompartmentMolecules",
             [](Simulation &sim, char *species, int number, const char *compartment) {
                 return smolAddCompartmentMolecules(sim.getSimPtr(), species, number,
                                                    compartment);
             })
        ...
        .def("addSurfaceMolecules",
             [](Simulation &sim, const char *species, MolecState state, int number,
                const char *surface, PanelShape panelshape, const char *panel,
                vector<double> &position) {
                 return smolAddSurfaceMolecules(sim.getSimPtr(), species, state, number,
        ...
        .def("setMoleculeStyle",
             [](Simulation &sim, const char *species, MolecState state, double size,
                char *color) {
                 ...
                 return smolSetMoleculeStyle(sim.getSimPtr(), species, state, size,
                                             &rgba[0]);
             })
        .def("setMoleculeStyle",
             [](Simulation &sim, const char *species, MolecState state, double size,
        ...
        .def("setMoleculeColor",
             [](Simulation &sim, const char *species, MolecState state, char *color) {
                 auto rgba = color2RGBA(color);
                 return smolSetMoleculeColor(sim.getSimPtr(), species, state, &rgba[0]);
             })
        ...
        .def("setBoundaryType",
             [](Simulation &sim, int dimension, int highside, char type) {
                 return smolSetBoundaryType(sim.getSimPtr(), dimension, highside, type);
             })
        ...
        .def("getSurfaceName",
             [](Simulation &sim, int surfaceindex, char *surface) {
                 return smolGetSurfaceName(sim.getSimPtr(), surfaceindex, surface);
             })
        ...
        .def(
            "setSurfaceAction",
            [](Simulation &sim, const char *surface, PanelFace face, const char *species,
               MolecState state, SrfAction action, const char *newspecies) {
                return smolSetSurfaceAction(sim.getSimPtr(), surface, face, species,
                                            state, action, newspecies);
            },
            "surface"_a, "face"_a, "species"_a, "state"_a, "action"_a,
            "newspecies"_a = "")
        ...
        //     int isinternal);
        .def("setSurfaceRate",
             [](Simulation &sim, const char *surface, const char *species,
                MolecState state, MolecState state1, MolecState state2, double rate,
                const char *newspecies, bool isinternal) {
                 return smolSetSurfaceRate(sim.getSimPtr(), surface, species, state,
        ...
        .def("addPanel",
             [](Simulation &sim, const char *surface, PanelShape panelshape,
                const char *panel, const char *axisstring, vector<double> &params) {
                 return smolAddPanel(sim.getSimPtr(), surface, panelshape, panel,
                                     axisstring, &params[0]);
             })
        ...
        .def("getPanelIndex",
             [](Simulation &sim, const char *surface, const char *panel) {
                 ...
                 return smolGetPanelIndex(sim.getSimPtr(), surface, &panelshape, panel);
             })
        ...
        .def("getPanelIndexNT",
             [](Simulation &sim, const char *surface, const char *panel) {
                 PanelShape panelshape{PanelShape::PSnone};
                 return smolGetPanelIndexNT(sim.getSimPtr(), surface, &panelshape, panel);
             })
        ...
        .def("getPanelName",
             [](Simulation &sim, const char *surface, PanelShape panelshape,
                int panelindex, char *panel) {
                 return smolGetPanelName(sim.getSimPtr(), surface, panelshape, panelindex,
                                         panel);
             })
        ...
        .def("setPanelJump",
             [](Simulation &sim, const char *surface, const char *panel1, PanelFace face1,
                const char *panel2, PanelFace face2, bool isbidirectional) {
                 return smolSetPanelJump(sim.getSimPtr(), surface, panel1, face1, panel2,
        ...
        .def("addSurfaceUnboundedEmitter",
             [](Simulation &sim, const char *surface, PanelFace face, const char *species,
                double emitamount, vector<double> &emitposition) {
                 return smolAddSurfaceUnboundedEmitter(sim.getSimPtr(), surface, face,
                                                       species, emitamount,
                                                       &emitposition[0]);
             })
        ...
        //     int reciprocal);
        .def("addPanelNeighbor",
             [](Simulation &sim, const char *surface1, const char *panel1,
                const char *surface2, const char *panel2, int reciprocal) {
                 return smolAddPanelNeighbor(sim.getSimPtr(), surface1, panel1, surface2,
                                             panel2, reciprocal);
             })
        ...
        //     shininess);
        .def("setSurfaceStyle",
             [](Simulation &sim, const char *surface, PanelFace face, DrawMode mode,
                double thickness, char *color, int stipplefactor, int stipplepattern,
                double shininess) {
                 array<double, 4> rgba = {0, 0, 0, 1.0};
                 graphicsreadcolor(&color, &rgba[0]);
                 return smolSetSurfaceStyle(sim.getSimPtr(), surface, face, mode,
                                            thickness, &rgba[0], stipplefactor,
                                            stipplepattern, shininess);
             })
        .def("setSurfaceStyle",
             [](Simulation &sim, const char *surface, PanelFace face, DrawMode mode,
                double thickness, array<double, 4> &rgba, int stipplefactor,
                int stipplepattern, double shininess) {
                 return smolSetSurfaceStyle(sim.getSimPtr(), surface, face, mode,
                                            thickness, &rgba[0], stipplefactor,
                                            stipplepattern, shininess);
             })
        ...
        .def("getCompartmentName",
             [](Simulation &sim, int compartmentindex, char *compartment) {
                 return smolGetCompartmentName(sim.getSimPtr(), compartmentindex,
                                               compartment);
             })
        ...
        .def("addCompartmentSurface",
             [](Simulation &sim, const char *compartment, const char *surface) {
                 return smolAddCompartmentSurface(sim.getSimPtr(), compartment, surface);
             })
        ...
        .def("addCompartmentPoint",
             [](Simulation &sim, const char *compartment, vector<double> &point) {
                 return smolAddCompartmentPoint(sim.getSimPtr(), compartment, &point[0]);
             })
        ...
        .def("addCompartmentLogic",
             [](Simulation &sim, const char *compartment, CmptLogic logic,
                const char *compartment2) {
                 return smolAddCompartmentLogic(sim.getSimPtr(), compartment, logic,
                                                compartment2);
             })
        ...
        .def("addReaction",
             [](Simulation &sim, const char *reaction,  // Name of the reaction.
                ...
                double              rate                // rate
                 ...
                 return smolAddReaction(sim.getSimPtr(), reaction, reactant1, rstate1,
                                        reactant2, rstate2, productSpecies.size(),
                                        &productSpecies[0], &productStates[0], rate);
             })
        ...
        .def("getReactionIndex",
             [](Simulation &sim, vector<int> &order, const char *reaction) {
                 return smolGetReactionIndex(sim.getSimPtr(), &order[0], reaction);
             })
        ...
        .def("getReactionIndexNT",
             [](Simulation &sim, vector<int> &order, const char *reaction) {
                 return smolGetReactionIndexNT(sim.getSimPtr(), &order[0], reaction);
             })
        ...
        .def("getReactionName",
             [](Simulation &sim, int order, int reactionindex, char *reaction) {
                 return smolGetReactionName(sim.getSimPtr(), order, reactionindex,
                                            reaction);
             })
        ...
        .def("setReactionRegion",
             [](Simulation &sim, const char *reaction, const char *compartment,
                const char *surface) {
                 return smolSetReactionRegion(sim.getSimPtr(), reaction, compartment,
                                              surface);
             })
        ...
        .def("setReactionProducts",
             [](Simulation &sim, const char *reaction, RevParam method, double parameter,
                const char *product, vector<double> &position) {
                 return smolSetReactionProducts(sim.getSimPtr(), reaction, method,
        ...
        .def("addPort",
             [](Simulation &sim, const char *port, const char *surface, PanelFace face) {
        ...
        .def("addPortMolecule",
             [](Simulation &sim, const char *port, int nmolec, const char *species,
                vector<vector<double>> &pos) {
                 std::vector<double *> ptrs;
                 for(auto &vec : pos) ptrs.push_back(vec.data());
                 return smolAddPortMolecules(sim.getSimPtr(), port, nmolec, species,
                                             ptrs.data());
             })
        ...
        .def("getPortMolecules",
             [](Simulation &sim, const char *port, const char *species, MolecState state,
                bool remove) {
                 return smolGetPortMolecules(sim.getSimPtr(), port, species, state,
                                             remove);
             })
        ...
        //         char *btype);
        .def("addLattice",
             [](Simulation &sim, const char *lattice, const vector<double> &min,
                const std::vector<double> &max, const vector<double> dx,
                const char *btype) {
                 return smolAddLattice(sim.getSimPtr(), lattice, &min[0], &max[0], &dx[0],
                                       btype);
             })
        ...
        .def("getLatticeName",
             [](Simulation &sim, int latticeindex, char *lattice) {
                 return smolGetLatticeName(sim.getSimPtr(), latticeindex, lattice);
             })
        ...
        .def("addLatticeMolecules",
             [](Simulation &sim, const char *lattice, const char *species, int number,
                vector<double> &lowposition, vector<double> &highposition) {
                 return smolAddLatticeMolecules(sim.getSimPtr(), lattice, species, number,
                                                &lowposition[0], &highposition[0]);
             })
        ...
        .def("addLatticeReaction",
             [](Simulation &sim, const char *lattice, const char *reaction,
                const int move) {
        ...
        .def("loadSimFromFile",
             [](Simulation &sim, const string &filepath, const char *flags) {
                 ...
                 return smolLoadSimFromFile(p.first.c_str(), p.second.c_str(), &psim,
                                            flags);
             })
        ...
        /* set accuracy */
        .def("setAccuracy",
             [](Simulation &sim, double accuracy) { sim.getSimPtr()->accur = accuracy; })
        .def("getAccuracy", [](Simulation &sim) { return sim.getSimPtr()->accur; })
        .def("runTimeStep",
